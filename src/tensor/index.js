...
export const normalize = () => handle =>
  [
    handle("[Number]", (input, { useState }) => {
      const [normals] = useState(() => ortho(input));
    ...
    handle("[[Number]]", (inputs, { useState }) => {
      const [normals] = useState(() => inputs.map(t => ortho(t)));
      const data = inputs.map((input, i) => shouldOrtho(input, normals[i]));
      return tensorTypeDef(TYPEDEF_NORMALIZED_NUMERIC_2D, reshape2d(data), {
        m: normals.map(({ m }) => m),
        s: normals.map(({ s }) => s)
      });
    })
...
const oneHotDefaultOptions = Object.freeze({
...
const oneHotTensor = (features, symbols) =>
  tf
    .tensor1d(
      [].concat(
        ...features
