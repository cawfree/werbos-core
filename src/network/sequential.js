...
export default (...layers) => burden =>
  burden(Sequential, ([xs, ys], { useState }) => {
    if (layers.length >= 2) {
      const {
      ...
      const [model] = useState(() =>
        layers.reduce((m, createLayer, i, orig) => {
    ...
    throw new Error(
      "A sequential network requires a minimum of two layers; an input layer, and an output layer."
    );
  });
